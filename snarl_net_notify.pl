#
# Snarl network notifications script
#
...
my $snp_prefix = "type=SNP#?version=1.0";
...
sub snarl_notify {
    ...
    if($sock) {
        ...
        for($n = 0; $n < 2; $n++) {
            print $sock "$snp_prefix#?action=notification#?app=$snarl_app#?class=1#?title=$title#?text=$desc#?timeout=$length\r\n";
            $lines = <$sock>;
            $code = get_code($lines);
...
sub snarl_register {    
    my $sock = get_sock();
    if($sock) {
        print $sock "$snp_prefix#?action=register#?app=$snarl_app\r\n";
        $line = <$sock>;
        $code = get_code($line);
        if($code == SNP_SUCCESS || $code == SNP_ERROR_ALREADY_REGISTERED) {
            print $sock "$snp_prefix#?action=add_class#?app=$snarl_app#?class=1#?title=IRC Notification\r\n";
            $line = <$sock>;
            $code = get_code($line);
            if($code == SNP_SUCCESS || $code == SNP_ERROR_CLASS_ALREADY_EXISTS) {
...
sub snarl_unregister {
    my $sock = get_sock();
    if($sock) {
        print $sock "$snp_prefix#?action=unregister#?app=$snarl_app\r\n";
        close($sock);
    }
}
...
weechat::register("snarl_net_notify", "eharmon, kinabalu <andrew\@mysticcoders.com>", $version, "GPL3", "Send Weechat notifications to Snarl", "", "");
